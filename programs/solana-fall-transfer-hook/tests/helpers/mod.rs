...
pub fn setup() -> (LiteSVM, Keypair, Address) {
    ...
    let mover_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../target/deploy/token_mover.so"
    );
    let mover_bytes =
        std::fs::read(mover_path).expect("token-mover must be built before tests run");
...
pub fn build_program_transfer_with_hook_ix(
    source_ata: &Pubkey,
    dest_ata: &Pubkey,
    mint: &Pubkey,
    owner: &Pubkey,
